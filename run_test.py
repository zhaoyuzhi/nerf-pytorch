...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            target = images[img_i] # read the image
            target = torch.Tensor(target).to(device)
            pose = poses[img_i, :3,:4] # read the extrinsics parameter (pose) of the image
            ...
            if N_rand is not None:
                ...
                coords = torch.reshape(coords, [-1,2])  # (H * W, 2), flatten operation

                # Only using N_rand pixels in current training step, due to memory limit
                select_inds = np.random.choice(coords.shape[0], size=[N_rand], replace=False)  # (N_rand,)
                select_coords = coords[select_inds].long()  # (N_rand, 2), it stands for randomly selected coordinates of the pixel space
                # print(select_coords, select_coords.shape)
                # tensor([[251, 196],
                #        [230, 145],
                #        [ 14,  52],
                #        ...,
                ...
                rays_o = rays_o[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3), find angle in the world space
                rays_d = rays_d[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3), find bias in the world space
